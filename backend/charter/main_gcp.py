"""
Cloud Functions (gen2) HTTP entrypoint for the Charter agent (GCP track).
...
def handler(request):
    """HTTP Cloud Function entrypoint for the Charter agent."""
    ...
    with observe():
        try:
            ...
            logger.info(f"Charter Cloud Function invoked with event keys: {list(event.keys())}")
            ...
            if not portfolio_data:
                ...
                logger.info(f"Charter: Loaded {len(portfolio_data['accounts'])} accounts")
            ...
            logger.error(f"Charter: Error in Cloud Function handler: {e}", exc_info=True)
            return (
